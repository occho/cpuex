...
static void declare_vars(void);
static void main_body(char*);
...
void convert(char *orig_buf) {
	...
	prefix();
	declare_vars();
	myputs("int main(int argc, char** argv) {");
	main_body(orig_buf);
	myputs("}");
...
#define INC_NUM 5
static const char *includes[INC_NUM] = {
	"<stdio.h>",
	"<stdlib.h>",
	"<stdint.h>",
	"<string.h>",
...
static void declare_vars(void) {
	include_part();
	myputs("uint32_t prom[ROM_NUM];");
	myputs("uint32_t ram[RAM_NUM];");
	myputs("int32_t reg[REG_NUM];");
	myputs("uint32_t freg[REG_NUM];");
}

static void main_body(char *buf) {
	while (mygets(asm_line, buf, COL_MAX) != NULL) {
